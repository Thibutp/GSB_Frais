 ...
 *
 * @param String $statut     V = visiteur | C = comptable
 * @param String $id         ID du visiteur | comptable
 * @param String $nom        Nom du visiteur | comptable
 * @param String $prenom     Prénom du visiteur | comptable
 *
 * @return null
 */
function connecter($statut, $id, $nom, $prenom)
{
...
{
    return preg_match('/[^0-9]/', (string)$valeur) == 0;
}
...
function estDateDepassee($dateTestee)
{
    ...
    $anActuel = $anneeTeste . $moisTeste . $jourTeste;
    return ($anActuel < $anPasse);
}
...
/**
 * Ajoute le texte REFUSE en début du Frais hors forfait si nécessaire 
 */
function ajouterRefuse($chaine)
{
    if(str_starts_with($chaine, "REFUSE "))
    {
        ajouterErreur("Ce frais est déjà refusé !");
    }
    else
    {
        $chaine = 'REFUSE '.$chaine;
    }
    return $chaine;
}
